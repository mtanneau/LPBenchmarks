...
julia --sysimage=$LPBDIR/JuliaLP.so --project=$LPBDIR $LPBDIR/src/gurobi.jl $LPBDIR/dat/plato/${inst[${SLURM_ARRAY_TASK_ID}-1]} > $LPBDIR/logs/gurobi/${inst[${SLURM_ARRAY_TASK_ID}-1]}.grb 2>&1