...
julia --sysimage=$LPBDIR/JuliaLP.so --project=$LPBDIR $LPBDIR/src/LP/mosek.jl $LPBDIR/dat/plato/${inst[${SLURM_ARRAY_TASK_ID}-1]} > $LPBDIR/logs/mittelman/mosek/${inst[${SLURM_ARRAY_TASK_ID}-1]}.msk 2>&1