...
julia --sysimage=$LPBDIR/JuliaLP.so --project=$LPBDIR $LPBDIR/src/ecos.jl $LPBDIR/dat/plato/${inst[${SLURM_ARRAY_TASK_ID}-1]} > $LPBDIR/logs/ecos/${inst[${SLURM_ARRAY_TASK_ID}-1]}.ecos 2>&1